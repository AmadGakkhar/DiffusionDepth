...
def save_semantic_map_from_sample(sample: dict, out_dir: str) -> bool:
    sem_keys = ['semantic_map', 'semantic', 'semantics', 'seg', 'segmentation']
    key_found = None
    for k in sem_keys:
    ...
    if sem_np.ndim == 3 and sem_np.shape[2] == 3:
        ...
        print(f"Saved semantic (RGB) from sample key '{key_found}'")
        return True

    # Otherwise assume label map [H, W]
    sem_np = sem_np.astype(np.int32)
    palette = (np.array(plt.get_cmap('tab20').colors) * 255.0).astype(np.uint8)
    ...
    print(f"Saved semantic (labels) from sample key '{key_found}'")
    return True
...
def main():
    ...
    # Include semantic (whatever key present) if any; model will ignore it but it's harmless
    for sk in ['semantic_map', 'semantic', 'semantics', 'seg', 'segmentation']:
        if sk in sample:
            batched_sample[sk] = to_batched(sample[sk])
            break
